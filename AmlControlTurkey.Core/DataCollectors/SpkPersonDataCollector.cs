...
{
    public class SpkPersonDataCollector() : IDataCollector
    {
        public async Task<List<AmlDataModel>> ProcessData(AmlDataEnum aml)
        {
            var list = await HttpRequestUtil.Get<List<SpkModel>>("https://ws.spk.gov.tr/IdariYaptirimlar/api/IslemYasaklari/Kisiler");
